...
var myGlobal = require('../common/myGlobal');
...
cc.Class({
    ...
    initGameData: function () {
        ...
        this.chooseIconPos = cc.v2(-1, -1) //控制小方块的位置
    ...
    handelMessage: function (message, data) {
        ...
        if (message == "exchange") {
            if (this.exchangeIcon(data.anchor)) {
                //如果可以交换，剩余步数-1
                this.remainderLabel.getComponent(cc.Label).string--;
        ...
        //检测
            ...
            else if (this.remainderLabel.getComponent(cc.Label).string <= 0) {    // 如果不能消除后，则游戏结束，结算分数
                console.log('游戏结束！');

                // 上传云服务器分数
                this.addData(this.score);

                this.resultBroad.active = true;
                this.resultBroad.getChildByName('score').getChildByName('scoreLabel').getComponent(cc.Label).string = this.score;
            }
        }
        //消除
    ...
    onmTouchMove: function (event) {
        if (this.isControl) {
            ...
            if (deltaDistance > 4900) {
                ...
                this.iconsTable[this.chooseIconPos.x][this.chooseIconPos.y].setPosition(cc.v2(touchLoc.x + this.deltaPos.x, touchLoc.y + this.deltaPos.y))
            }
        }
    },
    ...
    initDBCloud() {
        ...
        // 获取数据库引用
        myGlobal.wxDb = wx.cloud.database({
            //env: test03 - xs4hm
        });
        // 获取集合引用
        this.grade = myGlobal.wxDb.collection('grade');
        console.log('初始化数据库成功');

        wx.cloud.callFunction({
            name: 'getopenid',
            success: function (res) {
                myGlobal.openId = res.result.openid;
            },
            fail: console.error
        })
    ...
    addData(tempGrade) {
        ...
        myGlobal.wxDb.collection('grade').where({ _openid: myGlobal.openId }).get({
            success(res) {
                if (res.data.length > 0) {
                    let data = res.data[0];
                    let oldGrade = data.grade;
                    console.log('旧分数：' + oldGrade);
                    console.log('新分数：' + tempGrade);


                    if (oldGrade < tempGrade) {
                        // 新纪录 更新
                        console.log('刷洗了记录，准备更新数据');
                        console.log('_id:' + data._id);

                        //this.resultBroad.getChildByName('record').node.active = true;

                        self.uploadWXData(tempGrade);

                        myGlobal.wxDb.collection('grade').doc(data._id).update({
                            data: {
                                grade: tempGrade
                            },
                            success(res) {
                        ...
                        console.log('没有刷新纪录');
                        //this.resultBroad.getChildByName('record').node.active = false;

                    }
                } else {
                    // 无纪录添加数据
                    console.log('没有数据，要添加数据');

                    //this.resultBroad.getChildByName('record').node.active = true;

                    self.uploadWXData(tempGrade);

                    myGlobal.wxDb.collection('grade').add({
                        data: {
                            grade: tempGrade
                        },
                        success: function (res) {
                            console.log('插入分数数据成功！');
                            console.log(res);
    ...
    uploadWXData: function (bestHeight) { //上报到微信服务器：历史最高分
        ...
        kvDataList.push({ key: "yujx_MaxScore", value: "" + bestHeight });
        wx.setUserCloudStorage({
            KVDataList: kvDataList,
            success: (res) => {
                console.log('添加托管数据成功：' + res);
